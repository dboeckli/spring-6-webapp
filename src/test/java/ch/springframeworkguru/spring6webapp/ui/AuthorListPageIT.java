...
class AuthorListPageIT {
	...
	void testAuthorListPageLoads() {
		...
		for (WebElement row : authorRows) {
			...
			String firstName = wait.until(ExpectedConditions.presenceOfElementLocated(By.id("authorFirstName-" + id)))
				.getText();
			String lastName = wait.until(ExpectedConditions.presenceOfElementLocated(By.id("authorLastName-" + id)))
				.getText();
			authors.add(Author.builder().id(Long.parseLong(id)).firstName(firstName).lastName(lastName).build());
		}

		assertAll(() -> assertEquals("Author List", webDriver.getTitle()), () -> assertEquals(2, authorRows.size()),
